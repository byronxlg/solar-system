...
detections and the `window.__view` / `__kiosk` hooks still drive the sky. See
`src/devFlags.js`.
...
- Left: the sky, a 2D canvas looking down on the ecliptic. The Sun, the eight
  ...
  bottom-left gives its numbers and one thing worth knowing.
- Right: the kiosk. One always-on camera, mirrored. A mode banner and a
  ...
  - MAIN VIEW (blue): hold a Thumb Up to fly to the next body (Sun, then
    Mercury out to Neptune), Thumb Down for the previous one, two Open Palms
    to enter NAVIGATE, wave a hand (open hand swung side to side, tracked in
    `src/wave.js`) to take the tour, hold a Closed Fist to go back to the
    whole system.
  - NAVIGATE (teal): one hand with index finger touching thumb (Grab, detected
    from landmarks) pans the sky, two Pointing_Up fingertips pinching zoom, a
    held Closed Fist goes back to MAIN VIEW (the view stays where you left it).
    Moving the sky releases any focused body.
